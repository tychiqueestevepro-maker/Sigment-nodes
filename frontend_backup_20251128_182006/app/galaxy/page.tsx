...
interface Cluster {
    ...
    last_updated: string;
    created_at: string;
    description?: string; // Add description if available in API
}
...
export default function GalaxyViewPage() {
    ...
    const bubbles: Bubble[] = useMemo(() => {
        return pillars.map((pillar, index) => {
            ...
            const color = getColorForPillar(pillar.name);
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                ...
                                <div className="pt-6 mt-auto space-y-3">
                                    <button className="w-full py-3 bg-black text-white rounded-xl font-medium hover:bg-gray-800 transition-colors flex items-center justify-center gap-2">
                                        <FileText size={18} /> Treated Notes
                                    </button>
                                    <button className="w-full py-3 border border-gray-200 text-gray-500 rounded-xl font-medium hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all flex items-center justify-center gap-2">
                                        <Ban size={18} /> Refused
                                    </button>
                                </div>
                            </div>
                    ...
                    >
                        {bubbles.map((bubble) => (
                            <div
                                ...
                                className={`absolute rounded-full flex flex-col items-center justify-center text-white cursor-pointer shadow-2xl hover:scale-110 transition-transform duration-500 ease-out group ${bubble.size} ${bubble.color} ${bubble.position} ${bubble.zIndex}`}
                            >
