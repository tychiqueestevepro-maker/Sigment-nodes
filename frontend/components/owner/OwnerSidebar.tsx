...
import { SigmentLogo } from '../board/SigmentLogo';
...
export const Sidebar: React.FC = () => {
    ...
    // Updated links for Owner Space
    const menuItems: MenuItem[] = [
        { label: "Home", icon: <Home size={18} />, href: `/${orgSlug}/owner` },
        { label: "Galaxy View", icon: <Orbit size={18} />, href: `/${orgSlug}/owner/galaxy` },
        { label: "Review", icon: <FileCheck size={18} />, href: `/${orgSlug}/owner/review` },
        { label: "Analytics", icon: <BarChart2 size={18} />, href: `/${orgSlug}/owner/analytics` },
        { label: "Chat", icon: <MessageCircle size={18} />, href: `/${orgSlug}/owner/chat` },
        { label: "Groups", icon: <Users size={18} />, href: `/${orgSlug}/owner/groups` },
    ];
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-1">
                    ...
                    <div className="mt-auto pt-6">
                        ...
                        <Link
                            href={`/${orgSlug}/owner/archived`}
                            className={`flex items-center gap-3 w-full px-3 py-2 text-[14px] font-medium rounded-md transition-all duration-200 ${pathname === `/${orgSlug}/owner/archived`
                                ? 'bg-black text-white shadow-sm'
                                : 'text-gray-600 hover:bg-gray-50 hover:text-black'
                                } ${!isOpen ? 'justify-center' : ''}`}
                            title={!isOpen ? 'Archived' : ''}
                        >
                            <span className={pathname === `/${orgSlug}/owner/archived` ? 'text-white' : 'text-gray-500'}>
                                <Archive size={18} />
                            </span>
                    ...
                    {activeMenu === 'profile' && isOpen && (
                        <div className="absolute bottom-full left-2 mb-2 w-56 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden animate-in slide-in-from-bottom-2 fade-in duration-200">
                            ...
                            <div className="p-1 space-y-0.5">
                                <button
                                    onClick={() => {
                                        router.push(`/${orgSlug}/owner/admin`);
                                        setActiveMenu(null);
                                    }}
