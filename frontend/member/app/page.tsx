...
export default function MemberHomePage() {
    ...
    // Transform clusters to posts format
    const posts: Post[] = clusters.slice(0, 10).map((cluster: any, idx: number) => ({
        id: cluster.id,
        author: 'Board Member',
        role: 'Strategic Team',
        avatar: 'BM',
        color: getAvatarColor(idx),
        time: new Date(cluster.last_updated).toLocaleDateString(),
        category: cluster.pillar || 'GENERAL',
        content: cluster.title,
        likes: Math.floor(cluster.volume || 0),
        comments: Math.floor(cluster.impact_score || 0) % 20,
        impact: cluster.impact_score > 75 ? 'High Impact' : cluster.impact_score > 50 ? 'Medium Impact' : 'Low Impact',
    }));
    ...
    const galaxyFolders: GalaxyFolder[] = pillarsData.map((pillar: any) => ({
        id: pillar.id,
        name: pillar.name,
        count: clusters.filter((c: any) => c.pillar_id === pillar.id).length,
