...
export const UnifiedSidebar: React.FC = () => {
    ...
    // Fetch unread status for chat (Count)
    const { data: unreadStatus } = useQuery({
        queryKey: ['chatUnreadStatus', user?.id],
        queryFn: async () => {
            if (!user) return { unread_conversations_count: 0 };
            return api.get<{ unread_conversations_count: number }>('/chat/unread-status');
        },
        refetchInterval: 15000, // Faster poll for chat responsiveness
        enabled: !!user,
    });
